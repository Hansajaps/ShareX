 ...
 * 3. When a server is detected needing sync, the leader replicates missing files
...
public class ServerSyncService {
    ...
    /**
     * Check for servers requiring sync and replicate missing files to them
     */
    private void checkAndSyncServers() {
        try {
            ...
            List<String> serversNeedingSync = zooKeeperService.getServersRequiringSync();
            
            if (serversNeedingSync.isEmpty()) {
                return; // No servers need sync
            }
            
            logger.info("Found {} servers requiring sync: {}", serversNeedingSync.size(), serversNeedingSync);
            
            // Sync each server
            for (String serverId : serversNeedingSync) {
                if (serverId.equals(clusterConfig.getCurrentServer().getServerId())) {
                    // Skip self
                    logger.debug("Skipping sync for self ({})", serverId);
                ...
                try {
                    ...
                    logger.error("Failed to sync server {}: {}", serverId, e.getMessage());
                }
            }
            
        } catch (Exception e) {
            logger.debug("Error checking for servers requiring sync: {}", e.getMessage());
        }
    }
    ...
    private void syncServerFiles(String serverId) throws Exception {
        ...
        try {
            ...
            logger.info("Replication complete for server {}: {} / {} files", serverId, successCount, missingFiles.size());
    ...
    private byte[] getFileFromLeader(String leaderUrl, String filename) throws Exception {
        String url = leaderUrl + "/download?file=" + filename;
